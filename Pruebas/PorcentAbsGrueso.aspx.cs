...
{
    public partial class PorcentAbsGrueso : System.Web.UI.Page
    {
        ...
        protected void Select()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("Select CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) as Codigo,C127_A_Abs,C127_B_Abs from MPR_Det_Result_Prueba where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @Codigo", con);
                ...
                if (dr.Read())
                {
                    ...
                    sA.Text = dr["C127_A_Abs"].ToString();
                    sB.Text = dr["C127_B_Abs"].ToString();
                }
                else
                {
        ...
        protected void Insert()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("insert into MPR_Det_Result_Prueba(IdSolicPrueba,IdPrueba,FechaEmisionIndiv,C127_A_Abs,C127_B_Abs," +
               "C127_Abs_Results) values(@IdSolicPrueba,@IdPrueba,@FechaEmisionIndiv,@C127_A_Abs,@C127_B_Abs,@C127_Abs_Results)", con);
                ...
                cmd.Parameters.AddWithValue("@C127_A_Abs", sA.Value);
                cmd.Parameters.AddWithValue("@C127_B_Abs", sB.Value);
                cmd.Parameters.AddWithValue("@C127_Abs_Results", txtResult.Text);
        ...
        protected void Update()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("update MPR_Det_Result_Prueba set FechaEmisionIndiv=@FechaEmisionIndiv,C127_A_Abs=@C127_A_Abs,C127_B_Abs=@C127_B_Abs,C127_Abs_Results=@C127_Abs_Results where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @codigo", con);
                cmd.Parameters.AddWithValue("@codigo", txtId.Text);
                cmd.Parameters.AddWithValue("@FechaEmisionIndiv", DateTime.Now);
                cmd.Parameters.AddWithValue("@C127_A_Abs", sA.Value);
                cmd.Parameters.AddWithValue("@C127_B_Abs", sB.Value);
