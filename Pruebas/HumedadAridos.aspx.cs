...
{
    public partial class HumedadAridos : System.Web.UI.Page
    {
        ...
        protected void Select()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("Select CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) as Codigo,C566_W,C566_D from MPR_Det_Result_Prueba where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @Codigo", con);
                ...
                if (dr.Read())
                {
                    ...
                    sW.Text = dr["C566_W"].ToString();
                    sD.Text = dr["C566_D"].ToString();
                }
                else
                {
        ...
        protected void Insert()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("insert into MPR_Det_Result_Prueba(IdSolicPrueba,IdPrueba,FechaEmisionIndiv,C566_W,C566_D," +
               "C566_p_Result) values(@IdSolicPrueba,@IdPrueba,@FechaEmisionIndiv,@C566_W,@C566_D,@C566_p_Result)", con);
                ...
                cmd.Parameters.AddWithValue("@C566_W", sW.Value);
                cmd.Parameters.AddWithValue("@C566_D", sD.Value);
                cmd.Parameters.AddWithValue("@C566_p_Result", txtResult.Text);
        ...
        protected void Update()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("update MPR_Det_Result_Prueba set FechaEmisionIndiv=@FechaEmisionIndiv,C566_W=@C566_W,C566_D=@C566_D,C566_p_Result=@C566_p_Result where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @codigo", con);
                cmd.Parameters.AddWithValue("@codigo", txtId.Text);
                cmd.Parameters.AddWithValue("@FechaEmisionIndiv", DateTime.Now);
                cmd.Parameters.AddWithValue("@C566_W", sW.Value);
                cmd.Parameters.AddWithValue("@C566_D", sD.Value);
       ...
       #region formulas
        protected void CalcularHumedad()
        {
            double W = Convert.ToDouble(sW.Text);
            double D = Convert.ToDouble(sD.Text);
            double resta = W - D;
            double multiplicacion = 100 * resta;
            double division = multiplicacion / D;
