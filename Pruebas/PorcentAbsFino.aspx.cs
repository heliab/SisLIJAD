...
{
    public partial class PorcentAbsFino : System.Web.UI.Page
    {
        ...
        protected void Select()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("Select CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) as Codigo,C128_A_Abs,C128_S_Abs from MPR_Det_Result_Prueba where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @Codigo", con);
                ...
                if (dr.Read())
                {
                    ...
                    sA.Text = dr["C128_A_Abs"].ToString();
                    sS.Text = dr["C128_S_Abs"].ToString();
                }
                else
                {
        ...
        protected void Insert()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("insert into MPR_Det_Result_Prueba(IdSolicPrueba,IdPrueba,FechaEmisionIndiv,C128_A_Abs,C128_S_Abs," +
               "C128_Abs_Result) values(@IdSolicPrueba,@IdPrueba,@FechaEmisionIndiv,@C128_A_Abs,@C128_S_Abs,@C128_Abs_Result)", con);
                ...
                cmd.Parameters.AddWithValue("@C128_A_Abs", sA.Value);
                cmd.Parameters.AddWithValue("@C128_S_Abs", sS.Value);
                cmd.Parameters.AddWithValue("@C128_Abs_Result", txtResult.Text);
        ...
        protected void Update()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("update MPR_Det_Result_Prueba set FechaEmisionIndiv=@FechaEmisionIndiv,C128_A_Abs=@C128_A_Abs,C128_S_Abs=@C128_S_Abs,C128_Abs_Result=@C128_Abs_Result where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @codigo", con);
                cmd.Parameters.AddWithValue("@codigo", txtId.Text);
                cmd.Parameters.AddWithValue("@FechaEmisionIndiv", DateTime.Now);
                cmd.Parameters.AddWithValue("@C128_A_Abs", sA.Value);
                cmd.Parameters.AddWithValue("@C128_S_Abs", sS.Value);
