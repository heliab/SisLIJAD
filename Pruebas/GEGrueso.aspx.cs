...
{
    public partial class GEGrueso : System.Web.UI.Page
    {
        ...
        protected void Select()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("Select CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) as Codigo,C127_A_Ge,C127_B_Ge,C127_C_Ge from MPR_Det_Result_Prueba where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @Codigo", con);
                ...
                if (dr.Read())
                {
                    ...
                    sA.Text = dr["C127_A_Ge"].ToString();
                    sB.Text = dr["C127_B_Ge"].ToString();
                    sC.Text = dr["C127_C_Ge"].ToString();
                       }
                else
                {
        ...
        protected void Insert()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("insert into MPR_Det_Result_Prueba(IdSolicPrueba,IdPrueba,FechaEmisionIndiv,C127_A_Ge,C127_B_Ge," +
               "C127_C_Ge,C127_OD_Result) values(@IdSolicPrueba,@IdPrueba,@FechaEmisionIndiv,@C127_A_Ge,@C127_B_Ge,@C127_C_Ge,@C127_OD_Result)", con);
                ...
                cmd.Parameters.AddWithValue("@C127_A_Ge", sA.Value);
                cmd.Parameters.AddWithValue("@C127_B_Ge", sB.Value);
                cmd.Parameters.AddWithValue("@C127_C_Ge", sC.Value);
                cmd.Parameters.AddWithValue("@C127_OD_Result", txtResult.Text);
        ...
        protected void Update()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("update MPR_Det_Result_Prueba set FechaEmisionIndiv=@FechaEmisionIndiv,C127_A_Ge=@C127_A_Ge,C127_B_Ge=@C127_B_Ge,C127_C_Ge=@C127_C_Ge,C127_OD_Result=@C127_OD_Result where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @codigo", con);
                cmd.Parameters.AddWithValue("@codigo", txtId.Text);
                cmd.Parameters.AddWithValue("@FechaEmisionIndiv", DateTime.Now);
                cmd.Parameters.AddWithValue("@C127_A_Ge", sA.Value);
                cmd.Parameters.AddWithValue("@C127_B_Ge", sB.Value);
