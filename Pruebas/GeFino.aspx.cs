...
{
    public partial class GeFino : System.Web.UI.Page
    {
        ...
        protected void Select()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("Select CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) as Codigo,C128_A_Ge,C128_B_Ge,C128_C_Ge,C128_S_Ge from MPR_Det_Result_Prueba where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @Codigo", con);
                ...
                if (dr.Read())
                {
                    ...
                    sA.Text = dr["C128_A_Ge"].ToString();
                    sB.Text = dr["C128_B_Ge"].ToString();
                    sC.Text = dr["C128_C_Ge"].ToString();
                    sS.Text = dr["C128_S_Ge"].ToString();
                }
                else
                {
        ...
        protected void Insert()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("insert into MPR_Det_Result_Prueba(IdSolicPrueba,IdPrueba,FechaEmisionIndiv,C128_A_Ge,C128_B_Ge," +
               "C128_C_Ge,C128_S_Ge,C128_OD_Result) values(@IdSolicPrueba,@IdPrueba,@FechaEmisionIndiv,@C128_A_Ge,@C128_B_Ge,@C128_C_Ge,@C128_S_Ge,@C128_OD_Result)", con);
                ...
                cmd.Parameters.AddWithValue("@C128_A_Ge", sA.Value);
                cmd.Parameters.AddWithValue("@C128_B_Ge", sB.Value);
                cmd.Parameters.AddWithValue("@C128_C_Ge", sC.Value);
                cmd.Parameters.AddWithValue("@C128_S_Ge", sS.Value);
                cmd.Parameters.AddWithValue("@C128_OD_Result", txtResult.Text);
        ...
        protected void Update()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("update MPR_Det_Result_Prueba set FechaEmisionIndiv=@FechaEmisionIndiv,C128_A_Ge=@C128_A_Ge,C128_B_Ge=@C128_B_Ge,C128_C_Ge=@C128_C_Ge,C128_S_Ge=@C128_S_Ge,C128_OD_Result=@C128_OD_Result where CAST(IdSolicPrueba AS NVARCHAR) + '.' + CAST(IdPrueba AS NVARCHAR) + '.' + CAST(IdCalc AS NVARCHAR) = @codigo", con);
                cmd.Parameters.AddWithValue("@codigo", txtId.Text);
                cmd.Parameters.AddWithValue("@FechaEmisionIndiv", DateTime.Now);
                cmd.Parameters.AddWithValue("@C128_A_Ge", sA.Value);
                cmd.Parameters.AddWithValue("@C128_B_Ge", sB.Value);
        ...
        #region formulas
        protected void CalGravEspecCorr()
        {
